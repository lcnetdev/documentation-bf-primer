...
It is common to use both the direct and indirect relationship methods but it is important to employ them in consistent and clear ways. How the Library of Congress, for example, deploys which method has been heavily influenced by its need to convert from and to MARC Bibliographic data. Initially, the Library attempted to model all relationships using the direct method, but the richness and complexity of library bibliographic data revealed a number of shortcomings, top of which was loss of clarity in the data (or no improvement over the MARC data).

Most Work to Work relationships have been modelled using the indirect method. For those more familiar with MARC, these those found in the MARC 7XX and 8XX range. This has facilitated returning this information back to MARC but it has also deployed the most flexible option for the richest relationships found in bibliographic descriptions. The preceding example, for instance, merged a MARC 580 relationship note and two MARC 785 relationships together into a single BIBFRAME Relation resource.
...
Common practice has been to place all relationships at the bf:Work level. Said another way, relationships to other BIBFRAME resources (other than the direct ones noted above) are not expressed on the bf:Instance or bf:Item. No bf:Instance has a relationship to a bf:Hub; no bf:Hub has a relationship to a bf:Instance.

This is an implementation decision. Nothing in the model dictates that relationships cannot be at the bf:Instance or bf:Item level. But having them associated with the Work has created consistency and clarity.
